...
int main(void) {
    ...
    for (int i = 0; i < T; i++) {
        ...
        for (int j = 0; j < Q; j++) {  // (1 ≤ r ≤ N, 1 ≤ c ≤ M, 1 ≤ x ≤ 107)
            ...
            std::cin >> r >> c >> x;
            if (max_row[r - 1] < x) {
                if (radix[max_row[r - 1]] == true) {
                    if (maxs.count(max_row[r - 1]))
                        maxs.erase(max_row[r - 1]);
                    else
                        radix[max_row[r - 1]] = false;
                }
                max_row[r - 1] = x;
                if (radix[x] == true)
                    maxs.insert(x);
            ...
            if (max_col[c - 1] < x) {
                if (radix[max_col[c - 1]] == true) {
                    if (maxs.count(max_col[c - 1]))
                        maxs.erase(max_col[c - 1]);
                    else
                        radix[max_col[c - 1]] = false;
                }
                max_col[c - 1] = x;
                if (radix[x] == true)
                    maxs.insert(x);
